__author__ = 'runqi hou, hournqi@ad.unc.edu, onyen = hourunqi'
...
class Board:
    ...
    # Get the number of attacks made on the board.
    def get_attack_count(self):
        return self.attack_count

    # Get the number of hits on the board.
    def get_hit_count(self):
        return self.hit_count

    # Get the number of misses on the board.
    def get_miss_count(self):
        self.miss_count = self.attack_count - self.hit_count
...
class HumanPlayer:
    ...
    def take_turn(self):
        ...
        # Display the current statistics for each player.
        print("You have attacked", self.opponent.board.get_attack_count(), "times, with",self.opponent.board.get_hit_count(),
              "hits and", self.opponent.board.get_miss_count(), "misses.")
        # Get attack position.
        position = None
        while position is None:
            try:
...
# The ComputerPlayer class represents a player controlled by computer.
class ComputerPlayer:
    # Initialize the computer player with a name, a blank board, and a fleet of five boats.
    def __init__(self, player_name):
        self.player_name = player_name
        self.board = Board()
        self.fleet = [Boat("Aircraft Carrier", 5), Boat("Battleship", 4), Boat("Submarine", 3), Boat("Destroyer", 3), \
    ...
    # Positions all  boat randomly
    def position_boat(self, boat):
        # Random choose orientation.
        orientation = random.choice(['v', 'h'])

        # prime the loop and random choose the position of the boat.
        x = None
        while x is None:
            try:
    ...
    # This function allowed computer attacked randomly.
    def take_turn(self):
